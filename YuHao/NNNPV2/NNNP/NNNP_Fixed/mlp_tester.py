...
def train_eval(model, X_train, y_train, X_test, y_test, epochs, coeff, optimizer, learning_rate, is_shuffle):
    train_loss = []
    test_loss = []
    loss_fn = SquareError()
    for _ in range(epochs):
        ...
        train_loss.append(np.mean(model.train(
            X_train, y_train, optimizer=optimizer, learning_rate=learning_rate, coeff=coeff)))
...
def visualize(curve_dict, args):
    ...
    for i in args.hdims:
        hd_str += f'{i}_'
    plt.title(f'Dataset: {args.dataset}, Optimizer: {args.optimizer}, Test percentage: {args.testratio}\n Epochs: {args.epochs}, Learning Rate: {args.lr}, Hidden Dims: ' + hd_str)
    return fig
